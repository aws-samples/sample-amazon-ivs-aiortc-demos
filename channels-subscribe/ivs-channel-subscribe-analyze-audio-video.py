...
logger = logging.getLogger("ivs-channel-m3u8-audio-video-analyzer")
...
class AudioVideoChunkRecorder:
    ...
    def process_audio_frame(self, av_frame):
        ...
        if self.recording:
            self.recorded_audio_frames.append(av_frame)

    def encode_video_to_mp4(self, video_frames, audio_frames=None):
        """
        Encode video and audio frames to MP4 format in memory using PyAV

        Args:
            video_frames: List of video frames
            audio_frames: List of audio frames (optional)
        ...
        try:
            ...
            # Add audio stream if we have audio frames
            audio_stream = None
            if audio_frames:
            ...
            # Process audio frames if available
            if audio_frames and audio_stream:
                for i, audio_frame in enumerate(audio_frames):
                    try:
                        ...
                        new_audio_pts += audio_frame.samples
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="IVS Channel M3U8 Stream Audio/Video Analyzer with Bedrock")
    parser.add_argument(
        "--playlist-url",
        required=True,
...
def main():
    """Main function that handles M3U8 stream processing with audio/video analysis"""
    args = parse_args()

    logger.info("🎬 IVS Channel M3U8 Stream Audio/Video Analyzer")
    ...
    try:
        ...
        if not audio_stream:
            logger.warning("⚠️  No audio stream found")
            ...
            try:
                for frame in packet.decode():
                    if isinstance(frame, av.VideoFrame):
                        ...
                        # Process audio frame for recording if analyzer is available
                        if analyzer and recorder.recording:
