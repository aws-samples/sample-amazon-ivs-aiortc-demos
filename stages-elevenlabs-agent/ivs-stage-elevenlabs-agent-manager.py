#!/usr/bin/env python3
"""
IVS Stage ElevenLabs Agent Manager

Listens on an IVS Chat WebSocket for LAUNCH_ASSISTANT messages and spawns
ElevenLabs Conversational AI agent instances (ivs-stage-elevenlabs-agent.py)
for each participant. Supports configurable voice, LLM model, prompt,
...
logger = logging.getLogger("ivs-stage-elevenlabs-agent-manager")


class IVSStageElevenLabsAgentManager:
    def __init__(
        self,
        chat_room_arn: str,
        ws_endpoint: str,
        ...
        self.websocket = None
        self.manager_user_id = f"elevenlabs-agent-manager-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}"
    ...
    async def generate_stage_token(self, stage_arn: str, participant_id: str) -> str:
        try:
            response = self.ivs_realtime_client.create_participant_token(
                stageArn=stage_arn,
                userId=f"elevenlabs-agent-{participant_id}-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}",
    ...
    async def launch_elevenlabs_agent_instance(self, stage_arn: str, participant_id: str, config: Dict[str, Any] = None) -> tuple[bool, str]:
        ...
        try:
            ...
            script_path = os.path.join(os.path.dirname(__file__), "ivs-stage-elevenlabs-agent.py")
            cmd = [
                sys.executable,
                script_path,
                "--token",
            ...
            logger.info(f"✅ ElevenLabs Agent launched for participant {participant_id} (PID: {process.pid})")

            asyncio.create_task(self.monitor_instance(participant_id, process))
            return True, "SUCCESS"

        except Exception as e:
            logger.error(f"❌ Failed to launch ElevenLabs Agent for participant {participant_id}: {e}")
    ...
    async def monitor_instance(self, participant_id: str, process: subprocess.Popen):
        try:
            ...
            logger.info(f"🏁 ElevenLabs Agent for participant {participant_id} exited with code {return_code}")
        except Exception as e:
    ...
    async def handle_message(self, message: str):
        try:
            ...
            config = {
                "voiceId": data.get("voiceId") or cli.get("voiceId") or "JBFqnCBsd6RMkjVDRZzb",
                "llmModel": data.get("llmModel") or cli.get("llmModel") or "gemini-2.0-flash",
                "agentId": data.get("agentId") or cli.get("agentId"),
            ...
            success, error_code = await self.launch_elevenlabs_agent_instance(stage_arn, participant_id, config)

            if success:
                logger.info(f"🎉 Launched ElevenLabs Agent for {participant_id}")
                ...
                await self.send_error_response(
                    ...
                    stage_arn,
                    participant_id,
                    f"Failed to launch ElevenLabs Agent for participant {participant_id}",
                )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="IVS Stage ElevenLabs Agent Manager — spawns ElevenLabs Conversational AI agent instances via IVS Chat",
    )
    ...
    defaults.add_argument("--voice-id", default=None, help="Default ElevenLabs voice ID (default: JBFqnCBsd6RMkjVDRZzb)")
    defaults.add_argument("--llm-model", default=None, help="Default LLM model (default: gemini-2.0-flash)")
    defaults.add_argument("--agent-id", default=None, help="Default ElevenLabs agent ID (auto-creates if not set)")
...
async def main():
    ...
    elevenlabs_api_key = args.elevenlabs_api_key or os.getenv("ELEVENLABS_API_KEY")
    if not elevenlabs_api_key:
        logger.error("❌ ElevenLabs API key required. Use --elevenlabs-api-key or set ELEVENLABS_API_KEY env var")
        return

    logger.info("🎬 Starting IVS Stage ElevenLabs Agent Manager")
    ...
    manager = IVSStageElevenLabsAgentManager(
