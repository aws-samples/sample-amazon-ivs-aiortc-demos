#!/usr/bin/env python3
"""
IVS Real-Time Stage + ElevenLabs Conversational AI Agent

Subscribes to a participant's audio on an IVS stage, pipes it through
ElevenLabs' Conversational AI (STT → LLM → TTS in one WebSocket), and
...
# ── ICE timeout patch (same as Nova/GPT/Deepgram demos) ────────
ICE_TIMEOUT = 1
...
from elevenlabs_agent_manager import (
...
logger = logging.getLogger("ivs-stage-elevenlabs-agent")
...
# ── Utility functions (shared with Nova/GPT/Deepgram demos) ────
...
# ── WebRTC: subscribe to participant and pipe audio to ElevenLabs
...
async def subscribe_to_participant(
    ...
    def on_track(track: MediaStreamTrack):
        ...
        if track.kind == "audio":
            logger.info("🔊 Audio track received - processing through " "ElevenLabs Agent")
    ...
    async def _process_audio(track: MediaStreamTrack):
        ...
        logger.info(f"🎵 Starting audio processing task " f"(track state: {track.readyState})")
            ...
            while True:
                try:
                    ...
                    for resampled in resampler.resample(frame):
                        ...
                        if frame_count == 1:
                            logger.info(f"🎤 First audio frame sent to ElevenLabs " f"({len(audio_bytes)} bytes)")
                    ...
                    logger.info(f"Track state: {track.readyState}, " f"Connection: {pc.connectionState}")
                    continue
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="IVS Real-Time Stage + ElevenLabs Conversational AI Agent",
        epilog="""
Examples:
  %(prog)s --token eyJ... --subscribe-to user123
  %(prog)s --token eyJ... --subscribe-to user123 --agent-id abc123
  %(prog)s --token eyJ... --subscribe-to user123 --voice-id JBFqnCBsd6RMkjVDRZzb
  %(prog)s --token eyJ... --subscribe-to user123 --prompt "You are a pirate."
...
Environment variables:
  ELEVENLABS_API_KEY    ElevenLabs API key (alternative to --elevenlabs-api-key)
    ...
    parser.add_argument(
        "--token",
        required=True,
        help="IVS participant token with PUBLISH and SUBSCRIBE capabilities",
    )
    parser.add_argument(
        "--subscribe-to",
        required=True,
        help="Participant ID to subscribe to",
    )
    parser.add_argument(
        "--elevenlabs-api-key",
        default=None,
    ...
    parser.add_argument(
        "--agent-id",
        default=None,
        help=("Existing ElevenLabs Conversational AI agent ID. " "If not provided, an agent is created automatically."),
    )
    parser.add_argument(
        "--voice-id",
        default="JBFqnCBsd6RMkjVDRZzb",
        help=("ElevenLabs voice ID for auto-created agents " "(default: JBFqnCBsd6RMkjVDRZzb = George)"),
    )
    parser.add_argument(
        "--llm-model",
        default="gemini-2.0-flash",
        help="LLM model for agent reasoning (default: gemini-2.0-flash)",
    )
    parser.add_argument(
        "--prompt",
    ...
    parser.add_argument(
        "--multilingual",
        action="store_true",
        help="Enable automatic language detection — agent detects and responds in the user's language",
    )
    parser.add_argument(
        "--ice-timeout",
        type=int,
        default=1,
        help="ICE gathering timeout in seconds (default: 1)",
    )
    parser.add_argument(
...
async def main():
    ...
    logger.info("🎬 Starting IVS Stage + ElevenLabs Conversational AI Agent")
    ...
    if args.multilingual:
        logger.info("🌐 Multilingual: enabled (auto-detect and respond in user's language)")

    enable_frame_analysis = not args.disable_frame_analysis
    logger.info(f"🔍 Frame analysis: " f"{'enabled' if enable_frame_analysis else 'disabled'}")
    ...
    try:
        ...
        # Create agent audio track — ElevenLabs outputs 16kHz
        agent_audio_track = AgentAudioTrack(
        ...
        agent_manager = ElevenLabsAgentManager(
            ...
            participant_id=token_payload.get("jti", ""),
        ...
        # Subscribe to participant audio → ElevenLabs Agent
        ...
        logger.info("🎙️  ElevenLabs Conversational AI Agent is live! " "Press Ctrl+C to stop.")
