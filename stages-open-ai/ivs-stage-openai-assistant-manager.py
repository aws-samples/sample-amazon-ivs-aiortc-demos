...
class IVSStageOpenAIAssistantManager:
    ...
    def get_sample_payload(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "voice": "cedar",  # Optional: alloy, ash, ballad, coral, echo, sage, shimmer, verse, marin, cedar
            "vadMode": "server_vad",  # Optional: server_vad, semantic_vad
            "vadThreshold": 0.5,  # Optional: 0.0-1.0 for server_vad
    ...
    async def launch_openai_instance(self, stage_arn: str, participant_id: str, config: Dict[str, Any] = None) -> tuple[bool, str]:
        ...
        try:
            ...
            if config:
                voice = config.get("voice", "cedar")
                if voice in ["alloy", "ash", "ballad", "coral", "echo", "sage", "shimmer", "verse", "marin", "cedar"]:
                    cmd.extend(["--voice", voice])

                vad_mode = config.get("vadMode", "server_vad")
                if vad_mode in ["server_vad", "semantic_vad"]:
                    cmd.extend(["--vad-mode", vad_mode])

                    if vad_mode == "server_vad":
                        vad_threshold = config.get("vadThreshold", 0.5)
                        if isinstance(vad_threshold, (int, float)) and 0.0 <= vad_threshold <= 1.0:
                            cmd.extend(["--vad-threshold", str(vad_threshold)])
                    elif vad_mode == "semantic_vad":
                        vad_eagerness = config.get("vadEagerness", "medium")
                        if vad_eagerness in ["low", "medium", "high", "auto"]:
                            cmd.extend(["--vad-eagerness", vad_eagerness])

                # Add frame analysis options if specified
    ...
    async def handle_message(self, message: str):
        ...
        try:
            ...
            config = {
                ...
                "vadMode": data.get("vadMode", "server_vad"),
                "vadThreshold": data.get("vadThreshold", 0.5),
                "vadEagerness": data.get("vadEagerness", "medium"),
                "disableFrameAnalysis": data.get("disableFrameAnalysis", False),
                "bedrockModelId": data.get("bedrockModelId"),
                "bedrockRegion": data.get("bedrockRegion"),
            }
